...
public class UserService extends BaseService {
	...
	public List<User> getAllUsers() {
		...
		Response response = getRequestSpec().when().get().then().statusCode(200).contentType(ContentType.JSON)
				.body("$", hasSize(greaterThan(0))).extract().response();
	...
	private void validateUserList(List<User> users) {
		...
		long invalidUsers = users.stream().filter(user -> !user.isValidUser()).count();
		...
		log.info("User list validation completed. Total: {}, Invalid: {}", users.size(), invalidUsers);
	}
	...
	public Optional<User> findUserByUsername(String username) {
		...
		// Exact match first
		Optional<User> exactMatch = allUsers.stream().filter(user -> username.equals(user.getUsername())).findFirst();
		...
		if (caseInsensitiveMatch.isPresent()) {
			log.info("Found case-insensitive username match: {}", caseInsensitiveMatch.get().getUsername());
