...
public class CommentService extends BaseService {
	...
	public List<Comment> getCommentsByPostId(Integer postId) {
		...
		Response response = getRequestSpec().queryParam("postId", postId).when().get().then().statusCode(200)
				.contentType(ContentType.JSON).extract().response();

		List<Comment> comments = response.jsonPath().getList(".", Comment.class);

		// Validate all comments belong to the requested post
		List<Comment> validComments = comments.stream().filter(comment -> comment.belongsToPost(postId))
				.collect(Collectors.toList());

		if (validComments.size() != comments.size()) {
			log.warn("Found {} comments not belonging to post {}", comments.size() - validComments.size(), postId);
		}
