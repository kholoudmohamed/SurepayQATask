...
public class PostService extends BaseService {
    ...
    public List<Post> getPostsByUserId(Integer userId) {
        if (userId == null || userId <= 0) {
            ...
            List<Post> validPosts = posts.stream()
                .filter(post -> post.belongsToUser(userId))
                .collect(Collectors.toList());
            
            if (validPosts.size() != posts.size()) {
                log.warn("Found {} posts not belonging to user {}", 
                    posts.size() - validPosts.size(), userId);
            }
    ...
    private void validatePostList(List<Post> posts) {
        ...
        long invalidPosts = posts.stream()
            .filter(post -> !post.isValidPost())
            .count();
        ...
        log.info("Post list validation completed. Total: {}, Invalid: {}", 
            posts.size(), invalidPosts);
    }
